...
straddles_master$directional_lean <-ifelse(straddles_master$call_put_iv_spread > 0.01,"Bearish",
                                           ifelse(straddles_master$call_put_iv_spread < -0.01,"Bullish", "Neutral"))

# Market sector direction estimate
broad_index_sentiment <- mean(straddles_master$call_put_iv_spread)
ifelse(broad_index_sentiment > 0.01,"Bearish",
       ifelse(broad_index_sentiment < -0.01,"Bullish", "Neutral"))
...
sector_direction_list <- select(straddles_master, c(ticker,
                                                    underlying,
                                                    call_volume,
                                                    put_volume,
                                                    avg_iv,
